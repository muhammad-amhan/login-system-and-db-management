<!DOCTYPE html>	

<html>

<head>
	<link rel = "stylesheet" type = "text/css" href = "css/buttonTheme.css">
	<link rel = "stylesheet" type = "text/css" href = "css/dropdownTheme.css">
	<?php

	session_start();

	if($_SESSION['loggedin'] == false) {
		header('Location: loginForm.html');
	}

	$servername = "dragon.kent.ac.uk";
	$username = "co323";
	$password = "pa33word";
	$dbname = "co323";

	try {
		 $dbhandle = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
	}

	catch (PDOException $e) {
		  // The PDO constructor throws an exception if it fails
		  die('Error Connecting to Database: ' . $e->getMessage());
	}

	// Run the SQL query, and print error message if it fails.
	$sql = "SELECT tid AS teamID, category, division, clubID, venue FROM
	team JOIN club ON team.clubID = club.cid;";

	$query = $dbhandle -> prepare($sql);

	if ( $query -> execute() === FALSE ) {
		die('Error Running Query: ' . implode($query -> errorInfo(),' ')); 
	}
			
	// Put the results into a nice big associative array
	$results = $query-> fetchAll()

	?>
	
</head>

<body>
	<div class = "container">
		<div class = "header">
			<h1>TASK 3</h1>
		</div>
		
		<div class = "navBar">
			<hr id = "hr1">
			<input class = "menuBtn" type = "button" value = "Menu" onclick="window.location='menu.php';" />
			<input class = "logoutBtn" type = "button" value = "Logout" onclick="window.location='loginForm.html';" />
			</br>
		</div>
		
		<div class = "tableListContainer">
			<table border = "1px">
				<tr>
					<td>tid</td>
					<td>div</td>
					<td>cid</td>
					<td>cat</td>
					<td>venue</td>
				</tr>
			</table>

			<form action = "task4.php" method = "GET">
				<select name="dropDown">
					<?php
						$space = " - ";
						foreach ($results as $row) { 
							echo "<option value = ".$row['teamID'].">"
									 .$row['teamID'].$space
									 .$row['division'].$space
									 .$row['clubID'].$space
									 .$row['category'].$space
									 .$row['venue'].
								"</option>";
					}   
			?>
				</select>
				<input id = "submitBtn" type="submit">
			</form>
		</div>
		
		<div class = "bottomNav">
				<input type = "button" value = "Back" class = "backBtn" onclick="window.location='menu.php';" />
		</div>
		<div class = "footer">
			<hr id = "hr2">
			<footer>
				Database Management
			</footer>
		</div>
	</div>
</body>

</html>


